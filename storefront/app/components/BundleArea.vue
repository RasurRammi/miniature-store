...
<template>
  <UCollapsible v-model:open="isOpen" class="border border-default rounded-xl">
    <div
      class="flex items-center p-4 cursor-pointer bg-muted transition-colors rounded-xl gap-2"
    >
      <UIcon :name="isOpen ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'" class="size-6"/>
    ...
    <template #content>
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4 p-4">
        <ProductCard v-for="productV in bundle.productVariants.items" :key="productV.id" :productV="productV"/>

        <UCard variant="outline" :ui="{root: '', body: 'p-0 sm:p-0', footer: 'p-2 sm:px-2'}">
          <button
            @click="productDrawer.open()"
            class="aspect-square w-full cursor-pointer block">
            <span class="w-full h-full flex items-center justify-center">
              <UIcon name="i-lucide-plus" class="size-16 text-muted"/>
            </span>
          </button>
